...
## 第五步：整理内容与产出——规则在 OUTPUT_FORMAT.md，不在这个文件里

**抓完材料、准备整理成最终答案之前，先用 `read_file` 打开本技能目录下的
`OUTPUT_FORMAT.md`，读出完整正文**——不要凭对 Markdown 报告的固有印象直接开始写。这个文件
里是当前版本真实生效的整理规则和产出格式，**产出方式已经从"聊天里发 Markdown 正文"改成了
"生成 HTML 看板文件并用 `save_deliverable` 交付"**，如果没有实际打开这个文件读一遍，大概率
会凭旧习惯继续写 Markdown 段落——那是过时的格式，不要那样做。

`OUTPUT_FORMAT.md` 里有：人物维度四段结构的具体写法、账号与映射表近似匹配时的标注要求、
以及完整的 HTML 看板模板和交付步骤（`write_file`/`append_file` + `save_deliverable`）。
...
- 第五步指向的 `OUTPUT_FORMAT.md` 必须实际读取，不能跳过——这一步决定最终交付物是什么格式。
