...
统计区间开头一句话说明后，**按人分段**，每人固定四个部分，缺失的部分写"无"，不要省略标题：
...
不要输出按来源分列的表格（不要出现"GitHub 动态 / GitLab 动态 / Jira 动态"这样的列），来源信息
放在每条事项里，不是报告的组织结构。某人本期确实没有任何可核实活动，如实说明，不要编造。
